...
def display_select_mode():
    ...
    回答モードのラジオボタンを表示
    """
    # 回答モードを選択する用のラジオボタンを表示（サイドバーに配置）
    with st.sidebar:
        st.header("利用目的")
        ...
        <strong>【入力例】</strong><br>社員の育成方針に関するMTGの議事録
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""
        <span style="font-size: 1.05rem;"><strong>【「社内問い合わせ」を選択した場合】</strong></span>
        """, unsafe_allow_html=True)
        st.info("質問・要望に対して、社内文書の情報をもとに回答を得られます。")
        st.markdown("""
        <div style="background-color: white; padding: 10px; border-radius: 5px;">
        <strong>【入力例】</strong><br>人事部に所属している従業員情報を一覧化して
        </div>
        """, unsafe_allow_html=True)
